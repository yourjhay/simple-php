...
use App\Helper\Validation\ValidationHelpers\{ArrayHelpers, EnvHelpers};

/**
 * Fork from GUMP - A fast, extensible PHP input validation class.
 *
...
class BaseValidator
{
    // Singleton instance of$ BaseValidator
     ...
     * @param array $validators The BaseValidator validators
    ...
    {
        $baseValidator = self::get_instance();
        $baseValidator->validation_rules($validators);
        $baseValidator->set_fields_error_messages($fields_error_messages);

        if ($baseValidator->run($data) === false) {
            return $baseValidator->get_readable_errors();
        }
    ...
    {
        $baseValidator = self::get_instance();
        return $baseValidator->filter($data, $filters);
    }
